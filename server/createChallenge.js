...
app.get("/change_toggle", function (req, res) {
  if (req.params.toggle == 1) {
    connection.query(
      "UPDATE Alien SET graduate_toogle = 1 where id = ?",
      [res.user.id],
      function (err, results) {
        if (err) {
          console.error(err);
    ...
    connection.query(
      "UPDATE Alien SET graduate_toogle = 0 where id = ?",
      [res.user.id],
      function (err, results) {
        if (err) {
          console.error(err);
...
app.get("/main", function (req, res) {
  connection.query(
    "select * from Alien inner join Challenge on Alien.Challenge_id = Challenge.id order by accuredAuthCnt desc limit 50",
    function (err, results) {
      if (err) {
        console.error(err);
      }
...
app.post("/challenge_process", function (req, res) {
  ...
  connection.query(
    ...
    function (error, results) {
      ...
      connection.query(
        "INSERT INTO user_info_has_Challenge (user_info_id, Challenge_id) VALUES (?, ?)",
        [req.user.id, results.insertId]
      );
...
app.post("/create_alien", function (req, res) {
  connection.query(
    "INSERT INTO Alien (user_info_id, Challenge_id, alienName, Alien_image_url, auth_day) VALUES (?, ?, ?, ?, ?)",
    [req.user.id, req.challenge_id, "aa", "url1"]
  );
  connection.query(
